...
class GrayScaleDataProvider(BaseDataProvider):
    ...
    def __init__(self, nx, ny, **kwargs):
        ...
        self.ny = ny
        self.kwargs = kwargs
        rect = kwargs.get("rectangles", False) #不存在该key，则直接返回默认值
        if rect:
            self.n_class=3
...
def create_image_and_label(nx,ny, cnt = 10, r_min = 5, r_max = 50, border = 92, sigma = 20, rectangles=False):
    ...
    for _ in range(cnt): #随机生成20次数据
        a = np.random.randint(border, nx-border)
        b = np.random.randint(border, ny-border)
        r = np.random.randint(r_min, r_max)
        h = np.random.randint(1,255)

        y,x = np.ogrid[-a:nx-a, -b:ny-b] #随机中心点
        m = x*x + y*y <= r*r #随机中心点的圆
        mask = np.logical_or(mask, m)

        image[m] = h #随机位置生成1-255值

    label[mask, 1] = 1
    
    if rectangles: #引入方框噪声，上面的是圆圈噪声
        mask = np.zeros((nx, ny), dtype=np.bool)
    ...
    image += np.random.normal(scale=sigma, size=image.shape) #归一化
    image -= np.amin(image)
    image /= np.amax(image)
