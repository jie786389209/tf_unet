...
class Unet(object):
    ...
    def __init__(self, channels, n_class, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        logits, self.variables, self.offset = create_conv_net(self.x, self.keep_prob, channels, n_class, **kwargs) # 搭建网络

        self.cost = self._get_cost(logits, cost, cost_kwargs) # 定义损失函数

        self.gradients_node = tf.gradients(self.cost, self.variables) #定义梯度：损失函数 + 权重
        ...
        with tf.name_scope("cost"):
            ...
            if cost_name == "cross_entropy":
                ...
                if class_weights is not None:
                    ...
                    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=flat_logits, #定义损失函数
                                                                                     labels=flat_labels))
            elif cost_name == "dice_coefficient": #dice系数公式  2*交集 / 并即 = 2*X*Y / (X*X + Y*Y)
                eps = 1e-5
                prediction = pixel_wise_softmax(logits) #对像素点使用softmax
...
class Trainer(object):
    ...
    def train(self, data_provider, output_path, training_iters=10, epochs=100, dropout=0.75, display_step=1,
        ...
        with tf.Session() as sess:
            ...
            pred_shape = self.store_prediction(sess, test_x, test_y, "_init") #保存第一次训练后的预测
