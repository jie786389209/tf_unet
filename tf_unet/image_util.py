...
class BaseDataProvider(object):
    ...
    def _load_data_and_label(self):
        ...
        train_data = self._process_data(data) #归一化
        labels = self._process_labels(label) #把分割标志转换成两个维度的labels
    ...
    def _process_labels(self, label):
        if self.n_class == 2:
            ...
            if label.dtype != 'bool':
                label = label.astype(np.bool) #强制转换类型
    ...
    def _process_data(self, data):
        # normalization 归一化
        data = np.clip(np.fabs(data), self.a_min, self.a_max) #大于最大值的取最大值，小于最小值的取最小值
        data -= np.amin(data) #返回最小值
    ...
    def _post_process(self, data, labels): #可重载成自定义的版本
        """
    ...
    def __call__(self, n):  #使类实例能像函数那样被调用，而n是指生成多少次数据
